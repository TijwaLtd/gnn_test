...
def generate_index_main(root_dir, output, is_eq=0, graph_filename="graphs.pt", include_first=None, exclude_first=None):
    root_dir = os.path.abspath(root_dir)
    rows = []

    for dirpath, _, filenames in os.walk(root_dir):
        if graph_filename not in filenames:
            continue

        full_path = os.path.join(dirpath, graph_filename)
        ...
        if include_first is not None and first not in set(include_first):
            continue
        if exclude_first is not None and first in set(exclude_first):
            continue
    ...
    print(f"[OK] wrote {len(df)} rows -> {output}")
