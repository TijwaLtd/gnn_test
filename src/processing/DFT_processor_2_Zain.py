...
class DFTProcessor:
    ...
    def build_graph(
        ...
        **_,
        ...
        data.lattice  = torch.tensor(lattice_vectors, dtype=torch.float).unsqueeze(0)
    ...
    def process_directory(self):
        """Process all DFT calculations and save graphs to individual .pt files."""
        processed_count = 0
        for root, dirs, files in os.walk(self.dft_data_path):
            ...
            # Check if graphs.pt already exists in this directory
            output_file = os.path.join(root, 'graphs.pt')
            if os.path.exists(output_file):
                print(f"Skipping {root}: graphs.pt already exists.")
                processed_count += 1 # Count as processed if already exists
                continue
            ...
            if graph is not None:
                torch.save([graph], output_file) # Save a list containing the single graph
                print(f"Created and saved graph to {output_file}")
                processed_count += 1
        print(f"Finished processing. {processed_count} graphs saved.")
